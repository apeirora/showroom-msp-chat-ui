...
## Deploy via OCM Helm Chart

The `chat-ui-operator-ocm` Helm chart provides OCM release manifests (Component, ResourceGraphDefinition, and instance):

```bash
helm install chat-ui-ocm charts/chat-ui-operator-ocm \
  --namespace ocm-system \
  --set component.semver="v0.8.0" \
  --set operator.publicHost="chat-ui.example.com" \
  --set operator.publicScheme="https"
```
